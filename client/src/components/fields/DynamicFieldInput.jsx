...
function LineItemsInput({ value, onChange }) {
  ...
  function addRow() {
    onChange([...rows, { description: '', quantity: '', rate: '', paid: '' }]);
  }
      ...
      <table>
        ...
        <tbody>
          {rows.map((row, index) => {
            ...
            const rowDue = rowTotal - Number(row.paid || 0);
            return (
              <tr key={index}>
                <td>{index + 1}</td>
                <td>
                ...
                <td style={{ fontVariantNumeric: 'tabular-nums' }}>{rowTotal.toLocaleString()}</td>
                <td>
                  <input
                    type="number"
                    step="any"
                ...
                <td
                  style={{
                    ...
                    fontWeight: 600,
                    color: rowDue > 0 ? 'var(--danger)' : 'var(--success)',
                  }}
                >
